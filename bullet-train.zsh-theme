...
prompt_ruby() {
  if command -v asdf > /dev/null 2>&1; then
    version=$(asdf current ruby | sed -n -e 's/[^0-9.]*\([0-9.]*\).*/\1/p')
    if [[ -n $version ]]; then
      prompt_segment $BULLETTRAIN_RUBY_BG $BULLETTRAIN_RUBY_FG $BULLETTRAIN_RUBY_PREFIX" $version"
    fi
...
prompt_go() {
  ...
  if command -v asdf > /dev/null 2>&1; then
    version=$(asdf current golang | sed -n -e 's/[^0-9.]*\([0-9.]*\).*/\1/p')
    if [[ -n $version ]]; then
      prompt_segment $BULLETTRAIN_GO_BG $BULLETTRAIN_GO_FG $BULLETTRAIN_GO_PREFIX" $version"
    fi
  elif [[ (-f *.go(#qN) || -d Godeps || -f glide.yaml) ]]; then
